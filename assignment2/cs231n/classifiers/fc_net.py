...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dIn, dW1, db1 = affine_relu_backward(dHID, cache1)
        dW1 += self.reg * W1
        grads['W1'] = dW1
        grads['b1'] = db1
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        cache = {}
        out, cache_int = affine_forward(X, self.params['W1'], self.params['b1'])
        cache['affine1'] = cache_int
        
        if self.use_batchnorm:
            gamma, beta = self.params['gamma1'], self.params['beta1']
            out, cache['bnorm1'] = batchnorm_forward(out, gamma, beta, self.bn_params[0])
        out, cache_int = relu_forward(out)
        cache['relu1'] = cache_int
        if self.use_dropout:
            out, drop_cache = dropout_forward(out, self.dropout_param)
            cache['dropout1'] = drop_cache
        ...
        for i in range(2, self.num_layers):
            ...
            if self.use_batchnorm:
                ...
                out, cache['bnorm%d' % i] = batchnorm_forward(out, gamma, beta, self.bn_params[i - 1])
            out, cache['relu%d' % i] = relu_forward(out)
            if self.use_dropout:
                out,  cache['dropout%d' % i] = dropout_forward(out, self.dropout_param)
                                                                  
        scores, cache_last = affine_forward(out, self.params['W%d' % self.num_layers],  \
        ...
        dHID, dW, db = affine_backward(dscores, cache_last)
        
        dW += self.reg * cache_last[1]
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.use_batchnorm:
                dHID, grads['gamma%d' % i], grads['beta%d' % i] = batchnorm_backward(dHID, cache['bnorm%d' % i])
